...
本文是 B 站舆论监测桌面应用 Agent/MCP 接口的维护者契约。它描述 R1 已交付的只读能力；运行中的 `tools/list` 与每个工具的 Schema 始终是调用的最终依据。
...
| `bili_get_event_overview` | `event_id`、`mode` | 成员及 BV/采集状态、来源占比、情绪分布和分母、LLM 覆盖情况、时间范围、重复统计、快照 ID |
| `bili_search_event_comments` | `event_id`、`mode`、可选来源分析 ID/关键词/情绪、分页 | 限长、移除工具级用户名/UID 等标识字段的评论证据、来源 BV/分析 ID、分页信息、快照 ID |

事件口径复用产品现有事件评论池：成员分析缺失、未完成或评论采集未完成会被显式计入限制且不视为零评论来源。只有 `status=done` 且 `comment_collection_status=completed` 的成员评论可进入事件统计。工具分别返回全部可纳入事件评论分母上的 `raw_share` 与当前模式可匹配评论分母上的 `matched_share`；精确重复统计区分涉及评论数和超额数。零分母返回确定的 `0` 或空值，绝不返回 `NaN`。LLM 输出包含覆盖数、可用数和限制原因，不会回退为 NLP。
...
R2/R3 实现后须以实际 `tools/list`、Schema 和客户端证据继续更新本文。
