...
这是内部技术预览，不是已发布功能。它只提供 3 个 `stdio` 工具：列出已完成分析、读取单条分析概览、检索有限评论证据。

已完成大模型分析的记录会返回九类主情绪和五种表达风格；历史十分类结果不会混入当前大模型统计。
...
不要把正在写入且仍依赖 WAL 的原始数据库直接配置给此内部预览。`immutable=1` 只适用于经过确认的静态副本。
