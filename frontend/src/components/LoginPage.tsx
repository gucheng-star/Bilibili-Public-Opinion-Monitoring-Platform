    ...
    <main className="login-page">
      ...
      {step === 'welcome' ? (
        ...
        <div className="login-card">
          ...
          <div className="qr-frame">{qrUrl && !error && <img src={'https://api.qrserver.com/v1/create-qr-code/?size=200x200&data='+encodeURIComponent(qrUrl)} alt="B站登录二维码"/>}{!qrUrl && !error && <div className="pulse-dot" aria-label="正在生成二维码"/>}</div>
          <p className="login-status" aria-live="polite">{status}</p>
          <button onClick={()=>setStep('welcome')} className="login-back-button">← 返回登录方式</button>
        </div>
      )}
      </section>
      </div>
