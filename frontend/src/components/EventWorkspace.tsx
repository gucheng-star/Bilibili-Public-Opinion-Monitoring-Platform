...
import type { AnalysisMode, CommentData, FilterState, GroupAnalysisResult, GroupReanalysisStatus, KeywordItem, SentimentLLMV2, StyleDistributionV2, V2Emotion, V2Style } from '../types';
import { buildCommentTree } from '../utils/commentTree';
import { filtersSearchString } from '../utils/commentQuery';
import AISummaryCard from './AISummaryCard';
import AppErrorBoundary from './AppErrorBoundary';
import CommentEntryCard from './CommentEntryCard';
...
interface Props { groupId: number; initialFilters: FilterState; initialMode: AnalysisMode; onFiltersChange: (filters: FilterState) => void; onModeChange: (mode: AnalysisMode) => void; }

export default function EventWorkspace({ groupId, initialFilters, initialMode, onFiltersChange, onModeChange }: Props) {
  const [result, setResult] = useState<GroupAnalysisResult|null>(null); const [loading, setLoading] = useState(true); const [error, setError] = useState<string|null>(null); const [mode, setMode] = useState<AnalysisMode>(() => initialMode); const [filters, setFilters] = useState<FilterState>(() => initialFilters); const [keywords, setKeywords] = useState<KeywordItem[]>([]); const [keywordStatus, setKeywordStatus] = useState<'ready'|'loading'|'error'>('ready'); const [reanalysis, setReanalysis] = useState<GroupReanalysisStatus|null>(null); const [reanalyzeDialog, setReanalyzeDialog] = useState(false); const [showNlpDuringReanalysis, setShowNlpDuringReanalysis] = useState(false);
  const requestRef = useRef(0); const keywordRequestRef = useRef(0); const reanalysisRequestRef = useRef(0); const onFiltersChangeRef = useRef(onFiltersChange); const onModeChangeRef = useRef(onModeChange); const initialModeRef = useRef<AnalysisMode>(initialMode); const showNlpDuringReanalysisRef = useRef(false);
  ...
  const load = useCallback(async (nextMode: AnalysisMode) => { const request=++requestRef.current; setLoading(true); setError(null); try { const [data, status] = await Promise.all([getGroupResults(groupId,nextMode), getGroupReanalysisStatus(groupId)]); if(request!==requestRef.current)return; setResult(data); setMode(data.mode); initialModeRef.current=data.mode; onModeChangeRef.current(data.mode); setReanalysis(status.status==='analyzing'||status.status==='error'?status:null); setFilters(current => { if (current.sourceAnalysisId === 'all' || data.members.some(member => String(member.analysis_id) === current.sourceAnalysisId)) return current; const normalized = { ...current, sourceAnalysisId: 'all' }; onFiltersChangeRef.current(normalized); return normalized; }); } catch(reason) { if(request===requestRef.current)setError(reason instanceof Error?reason.message:'读取事件结果失败'); } finally { if(request===requestRef.current)setLoading(false); } }, [groupId]);
  useEffect(()=>{ if(initialModeRef.current===initialMode)return; initialModeRef.current=initialMode; setMode(initialMode); void load(initialMode); },[initialMode,load]);
  useEffect(()=>{ reanalysisRequestRef.current++; setResult(null); setMode(initialModeRef.current); setReanalysis(null); setReanalyzeDialog(false); setShowNlpDuringReanalysis(false); void load(initialModeRef.current); },[groupId,load]);
  ...
  const llmV2 = useMemo(()=>{const emotion:SentimentLLMV2={neutral:0,joy:0,trust:0,anticipation:0,surprise:0,anger:0,sadness:0,fear:0,disgust:0};const style:StyleDistributionV2={plain:0,sarcasm:0,meme:0,rhetorical:0,hyperbole:0};filtered.forEach(comment=>{const label=comment.sentiment_llm_label as V2Emotion;const expression=comment.sentiment_llm_style as V2Style;if(V2_EMOTIONS.includes(label))emotion[label]++;if(V2_STYLES.includes(expression))style[expression]++;});return {emotion,style};},[filtered]);
  ...
  const entryTreeStats = useMemo(()=>{ const roots=buildCommentTree(filtered,commentKeys); return { rootCount:roots.length, replyCount:filtered.length-roots.length }; },[filtered,commentKeys]);
  ...
  const pendingLlmComments = useMemo(()=>result?.comments.filter(comment=>comment.sentiment_llm_schema_version!==2).length||0,[result]);
  const sourceCoverage = result?.llm_readiness?.source_coverage || [];
  const sourceDistribution = (result?.source_distribution || []).map(source => ({ ...source, ...sourceCoverage.find(item => item.analysis_id === source.analysis_id) }));
  ...
  const filtersSearch = filtersSearchString(filters);
  const commentDetailSearch = mode === 'llm'
    ? `${filtersSearch}${filtersSearch ? '&' : '?'}mode=llm`
    : filtersSearch;
  return <AppErrorBoundary diagnosticState={{ route:'/', view_type:'group', group_id:groupId, analysis_mode:mode, loading, reanalyzing:activeReanalysis, keyword_status:keywordStatus, active_filter_fields:activeFilterFields(filters) }}><>{error&&<div className="app-alert app-alert--error" role="alert">{error}</div>}<div className="app-alert app-alert--status">舆情事件 · {mode==='llm'?'大模型情绪与表达风格':'NLP 三分类'} · 共 {result.total_comments.toLocaleString()} 条评论</div><EventInfo result={result}/><FilterBar filters={filters} onApply={applyFilters} availableRegions={regions} mode={mode} duplicateStatistics={result.duplicate_statistics} duplicateGroups={duplicateGroups} originalCount={result.comments.length} duplicateRetainedCount={duplicateFiltered.length} sources={result.members} llmSchemaVersion={mode==='llm'?2:undefined}/>{mode==='nlp'&&!result.llm_readiness?.ready&&<div className="app-alert app-alert--status mt-4" role="status">大模型情绪与表达风格覆盖 {sourceCoverage.reduce((sum,item)=>sum+item.v2_completed_comments,0).toLocaleString()} / {sourceCoverage.reduce((sum,item)=>sum+item.v2_total_comments,0).toLocaleString()} 条评论；待补齐来源：{sourceCoverage.filter(item=>item.v2_pending_comments>0).map(item=>`${item.video_title||item.bv}（${item.v2_pending_comments} 条）`).join('、')||'无'}{reanalysis?.errors.length?`。失败来源：${reanalysis.errors.map(item=>item.video_title).join('、')}`:''}</div>}<div className="card-enter mt-4"><AISummaryCard scope={{kind:'group',id:groupId}} filters={filters} matchedCount={filtered.length} mode={mode}/></div><div className="card-enter mt-4"><SentimentChart positive={sentiment.positive} negative={sentiment.negative} neutral={sentiment.neutral} mode={mode} llmV2={mode==='llm'?llmV2:null} onModeChange={changeMode} reanalysis={reanalysisPanel} backgroundReanalysis={backgroundReanalysis}/></div><div className="card-enter mt-4"><HeatTimeline timeline={heat.timeline} hourlyDistribution={heat.hourly_distribution} peakHour={heat.peak_hour} peakCount={heat.peak_count}/></div><div className="card-enter mt-4"><WordCloudCard keywords={keywords} status={keywordStatus} scopeKey={`group:${groupId}`}/></div><div className="space-y-4 mt-4 distribution-chart-stack"><div className="card-enter"><GenderChart male={gender.male} female={gender.female} unknown={gender.unknown}/></div><div className="card-enter"><RegionMap data={region}/></div></div><div className="card-enter mt-4"><SourceDistributionPanel sources={sourceDistribution} filteredComments={filtered} mode={mode}/></div><div className="card-enter mt-4"><CommentEntryCard total={filtered.length} rootCount={entryTreeStats.rootCount} replyCount={entryTreeStats.replyCount} to={`/analysis-groups/${groupId}/comments`} search={commentDetailSearch}/></div>{reanalyzeDialog&&<div className="reanalyze-dialog" onClick={()=>setReanalyzeDialog(false)}><div className="reanalyze-dialog__panel" role="dialog" aria-modal="true" aria-labelledby="event-reanalyze-title" onClick={event=>event.stopPropagation()}><h3 id="event-reanalyze-title">补齐事件的大模型情绪与表达风格分析</h3><p className="reanalyze-dialog__copy">将仅分析 {pendingLlmComments.toLocaleString()} 条尚未完成的评论；已具备合法大模型情绪与表达风格标签的评论会保留，并仅作为同源回复上下文使用。</p><p className="reanalyze-dialog__notice">涉及来源：{(result.llm_readiness?.missing_members||[]).map(member=>member.video_title||member.bv).join('、')||'部分来源'}。调用设置中的情绪分析模型，可能产生少量费用。</p><div className="reanalyze-dialog__actions"><button type="button" onClick={()=>setReanalyzeDialog(false)} className="btn btn-ghost">取消</button><button type="button" onClick={()=>void confirmReanalysis()} className="btn btn-primary">确认补齐分析</button></div></div></div>}</></AppErrorBoundary>;
}
