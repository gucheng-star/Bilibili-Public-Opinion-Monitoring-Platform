...
import type { AnalysisMode, CommentData, SentimentLLM, SourceDistributionItem } from '../types';
import './DataPanels.css';

interface Props { sources: SourceDistributionItem[]; filteredComments: CommentData[]; mode: AnalysisMode; }

const EMOTION_LABELS: Record<string, string> = { positive: '正面', negative: '负面', neutral: '中性', joy: '喜悦', support: '支持', anticipation: '期待', surprise: '惊讶', anger: '愤怒', sadness: '悲伤', concern: '担忧', disgust: '厌恶', sarcasm: '反讽' };
...
export default function SourceDistributionPanel({ sources, filteredComments, mode }: Props) {
  ...
  const rows = useMemo(() => {
    ...
    return sources.map(source => {
      ...
      const sentimentLlm: SentimentLLM = { neutral:0, joy:0, support:0, anticipation:0, surprise:0, anger:0, sadness:0, concern:0, disgust:0, sarcasm:0 };
      comments.forEach(comment => {
        ...
        if (label in sentimentLlm) sentimentLlm[label] += 1;
      });
  ...
  return <section className="card source-distribution" aria-labelledby="source-distribution-title">
    ...
    <div className="source-distribution__table-wrap"><table className="source-distribution__table"><thead><tr><th>视频</th><th>BV</th><th>原始评论</th><th>筛选后</th><th>占评论池</th>{view === 'compare' && <th>{mode === 'llm' ? '主要十分类标签' : '主要情感'}</th>}<th>LLM</th></tr></thead><tbody>{rows.map(row => { const sentiments = mode === 'llm' ? row.filteredSentimentLlm : row.filteredSentiment; const main = row.filtered ? Object.entries(sentiments).sort((left, right) => Number(right[1]) - Number(left[1]))[0] : undefined; return <tr key={row.analysis_id}><td title={row.video_title}>{row.video_title || '-'}</td><td>{row.bv}</td><td>{row.total_comments.toLocaleString()}</td><td>{row.filtered.toLocaleString()}</td><td>{total ? `${(row.filtered / total * 100).toFixed(1)}%` : '0%'}</td>{view === 'compare' && <td>{main ? `${EMOTION_LABELS[main[0]] || main[0]} ${main[1]} 条` : '-'}</td>}<td>{row.llm_ready === false ? '未完成' : '可用'}</td></tr>; })}</tbody></table></div>
  </section>;
}
