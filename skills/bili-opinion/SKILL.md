...
1. 若用户尚未安装应用，提供 [GitHub Releases](https://github.com/gucheng-star/Bilibili-Public-Opinion-Monitoring-Platform/releases) 链接，请用户下载 `BiliOpinionMonitor-<版本>-windows-x64.exe` 到可写目录并自行启动；不要替用户下载、运行未知文件或绕过 Windows 的安全提示。
2. 请用户在桌面应用“设置 → Agent / MCP”点击“生成 Agent 快照”，再点击“复制 JSON 配置”，并将完整 JSON 粘贴到对话中。快照由用户主动生成；没有 JSON 时，不猜测 EXE 或数据库路径，也不注册 MCP。
3. 用户明确授权后，按下节注册；重新连接后先验证工具，再开始只读研判。

## Codex 注册（仅在明确授权后）

仅当用户同时提供桌面应用“复制 JSON 配置”的完整内容，并明确允许新增或更新 `bili-opinion-readonly` 时，才可修改本机 Codex 配置。若无法读取本 Skill，停止并要求用户提供，不能注册。
...
- 注册命令的等价形式为：`codex mcp add bili-opinion-readonly --env "BILI_MCP_DB_PATH=<JSON 中的数据库路径>" -- "<JSON 中的主 EXE 路径>" --mcp-stdio`。优先以结构化参数调用，保留包含空格和中文的原始路径。
- 注册后重新连接；在新会话中先验证 `tools/list` 和 `bili_get_data_source_info`。无法刷新当前会话时，说明需要用户新开或重连会话，不要通过修改其他配置强行加载。
- 未获明确授权时，请用户提供桌面应用复制的完整 JSON，并明确写出“我授权你新增或更新 `bili-opinion-readonly`”；在此之前不写入、删除或替换客户端配置。
